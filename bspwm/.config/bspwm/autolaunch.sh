...
if [[ $(xrandr -q | grep -E "^${RIGHT_MONITOR} connected") && ! $(xrandr -q | grep "^${MAIN_MONITOR} connected") ]]; then
  # present_mode: set screen blank time to 10 minutes for present monitor setup
  echo $LOG_PREFIX "Present mode"
  feh --no-fehbg --bg-fill $HOME/dotfiles/wallpapers/debian.jpg --bg-fill $HOME/dotfiles/wallpapers/debian.jpg &
  xset dpms 600 900 1200
elif [[ $(xrandr -q | grep "^${MAIN_MONITOR} connected") ]]; then
  # set screen blank time to 10 minutes for main monitor setup
  xset dpms 600 900 1200
  if [[ "$1" == 0 ]]; then
  ...
  if [[ $(xrandr -q | grep -E "^${RIGHT_MONITOR} connected") ]]; then
    ...
    echo $LOG_PREFIX "Only main monitor connected"
    feh --no-fehbg --bg-fill $HOME/dotfiles/wallpapers/debian.jpg &
  fi
else
  echo $LOG_PREFIX "Internal mode"
  betterlockscreen -u ~/dotfiles/wallpapers/japan01.png --fx dim,pixel &
  feh --no-fehbg --bg-fill $HOME/dotfiles/wallpapers/debian.jpg &
fi
